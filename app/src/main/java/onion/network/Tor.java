...
public class Tor {
    ...
    private static String torname = "ftor";
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    ...
                    log("arch " + arch);
                    arch = arch.substring(0, 3).toUpperCase();
                    log("arch " + arch);
                    ...
                    InputStream res = context.getResources().openRawResource(R.raw.bin);
                    LZMACompressorInputStream lzma = new LZMACompressorInputStream(res);
                    TarArchiveInputStream tar = new TarArchiveInputStream(lzma);

                    int i = 0;
                    //int ii = 4;

                    for (TarArchiveEntry e = tar.getNextTarEntry(); e != null; e = tar.getNextTarEntry()) {

                        i++;
                        String statpre = "Loading " + new String(new char[]{(char) (i + 'A')});
                        stat(statpre);
                        ...
                        if (v.equals(n)) {
                            ...
                            int it = 0;
                            while (p < s) {
                                int c = tar.read(b, 0, Math.min(b.length, s - p));
                                os.write(b, 0, c);
                                p += c;
                                if (it++ % 8 == 0) stat(statpre + " " + p * 100 / s + "%");
                            }
    ...
    synchronized public static Tor getInstance(Context context) {
        if (instance == null) {
